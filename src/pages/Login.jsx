...
function Login() {
  ...
  const scanFace = async () => {
    ...
    const x = faceapi.matchDimensions(canvasRef.current, videoRef.current);
    // console.log(x);
    const faceApiInterval = setInterval(async () => {
      ...
      const results = labeledFaceDescriptors.descriptors.map((d) =>
        faceMatcher.findBestMatch(d)
      );

      for(let i=0;i<results.length;i++){
        if(results[i]._label!=='unknown'){
          console.log(tempAccount[i].picture)
        }
      }
...
for(let i=0;i<tempAccount.length;i++){
  // console.log("value of i",i, tempAccount[i])
  
    const imgPath =
      tempAccount[i]?.type === "CUSTOM"
        ? tempAccount[i].picture
        : // : import.meta.env.DEV
          // ? `/temp-accounts/${tempAccount.picture}`
          // : `/react-face-auth/temp-accounts/${tempAccount.picture}`;
          `/temp-accounts/${tempAccount[i].picture}`;
// console.log(tempAccount.picture);
...
if (detections) {
  ...
  console.log(descriptions)
    ...
    // console.log(detections,"kya")

   
    // console.log(descriptions[0])
    return new faceapi.LabeledFaceDescriptors(tempAccount[0].id, descriptions);
  }
